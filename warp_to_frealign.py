...
from processing_functions import generate_new_classes
...
generate_new_classes(class_number=class_number, pixel_size = pixel_size, low_res = low_res_limit, high_res = high_res_limit_initial)
...
def refine_2d_subjob(process_number, round=0, particles_per_process = 1, high_res_limit = high_res_limit_initial, class_fraction = class_fraction):
    ...
    input = "\n".join([
        "single_stacked.mrcs", # Input MRCS stack
        "classes_{0}.par".format(round), # Input Par file
...
def merge_par_files(cycle, process_count=process_count):
...
for cycle_number in range(cycle_count):
    ...
    merge_par_files(cycle_number, process_count=process_count)

print("Long 2D classifications to incorporate all particles")
print("======================================")
# 5 cycles of finalizing refinement to clean it up.
for cycle_number in range(cycle_count, cycle_count+5):
    high_res_limit = high_res_limit_final
    print("High Res Limit: {0:.2}".format(high_res_limit))
    print("Fraction of Particles: {0:.2}".format(1.0))
    pool = multiprocessing.Pool(processes=process_count)
    refine_job = partial(refine_2d_subjob, round=cycle_number, particles_per_process=particles_per_process, high_res_limit=high_res_limit, class_fraction=1.0)
    results_list = pool.map(refine_job, range(process_count))
    print(results_list[0].decode('utf-8'))
    merge_2d_subjob(cycle_number)
    merge_par_files(cycle_number, process_count=process_count)
